...
def index():
    if request.method == "POST":
        ...
        if face_count != 1:
            return jsonify({"error": f"Detected {face_count} faces. Please upload an image with exactly one face."})
        ...
        return jsonify({"segmented_image": encoded_img})
